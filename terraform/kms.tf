...
resource "aws_kms_key" "redacted_output" {
  description             = "Encrypts redacted output in redacted-output"
  deletion_window_in_days = 30
  enable_key_rotation     = true
...
resource "aws_kms_alias" "redacted_output" {
  name          = "alias/patient-deid-redacted-output"
  target_key_id = aws_kms_key.redacted_output.key_id
}

resource "aws_kms_key" "review_artifacts" {
  description             = "Encrypts review artifacts in review-artifacts"
  deletion_window_in_days = 30
  enable_key_rotation     = true
...
resource "aws_kms_alias" "review_artifacts" {
  name          = "alias/patient-deid-review-artifacts"
  target_key_id = aws_kms_key.review_artifacts.key_id
}